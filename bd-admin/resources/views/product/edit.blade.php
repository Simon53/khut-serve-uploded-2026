    ...
    <select name="main_menu_id" id="mainMenuSelect" class="form-control" required>
        ...
        @foreach($mainMenus as $main)
            <option value="{{ $main->id }}"
                {{ $product->main_menu_id == $main->id ? 'selected' : '' }}>
                {{ $main->name }}
            </option>
        @endforeach
    </select>
...
<div class="form-group">
    ...
    <select name="sub_menu_id" id="subMenuSelect" class="form-control">
        ...
        @foreach($subMenus->where('main_menu_id', $product->main_menu_id) as $sub)
            <option value="{{ $sub->id }}"
                {{ $product->sub_menu_id == $sub->id ? 'selected' : '' }}>
                {{ $sub->name }}
            </option>
        @endforeach
    </select>
...
<div class="form-group">
    ...
    <select name="child_menu_id" id="childMenuSelect" class="form-control">
        ...
        @foreach($childMenus->where('sub_menu_id', $product->sub_menu_id) as $child)
            <option value="{{ $child->id }}"
                {{ $product->child_menu_id == $child->id ? 'selected' : '' }}>
                {{ $child->name }}
            </option>
        @endforeach
    </select>
...
$('#loadMoreGallery').click(function () {
   ...
   $(document).ready(function () {
    ...
    if ($('#subMenuSelect option').length > 1) {
        $('#subMenuSelect').prop('disabled', false);
    } else {
        $('#subMenuSelect').prop('disabled', true);
    }

    if ($('#childMenuSelect option').length > 1) {
