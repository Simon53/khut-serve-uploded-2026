...
class CategoryController extends Controller
{
    ...
    public function productsByMenu()
    {
        ...
        return view(
            'product-categories.partials.products',
            compact('products', 'stocks')
        );
    }
    ...
    public function listBySlug($slug)
    {
        ...
        $subIds = $category->subMenus
            ->pluck('id')
            ->toArray();

        $childIds = ChildMenu::whereIn(
                'sub_menu_id',
                $subIds
            )
            ->pluck('id')
    ...
    public function listById($id)
    {
        ...
        if ($routeName === 'subcategory.list') {
            ...
            $products = Product::where(
                ...
                ->get(); // IMPORTANT: paginate(40) থেকে get()


            $banner = CategoryBanner::where(
                'main_menu_id',
                $category->main_menu_id
            )->first();
