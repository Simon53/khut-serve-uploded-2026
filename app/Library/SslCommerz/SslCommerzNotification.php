...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function setRequiredInfo(array $info)
    {
        ...
        // Check if URLs are provided in the request data first (allow override)
        if (isset($info['success_url']) && !empty($info['success_url'])) {
            $this->data['success_url'] = $info['success_url']; // Use provided success URL
        } else {
            $this->setSuccessUrl();
            $this->data['success_url'] = $this->getSuccessUrl(); // Use config default
        }
        
        if (isset($info['fail_url']) && !empty($info['fail_url'])) {
            $this->data['fail_url'] = $info['fail_url']; // Use provided fail URL
        } else {
            $this->setFailedUrl();
            $this->data['fail_url'] = $this->getFailedUrl(); // Use config default
        }
        
        if (isset($info['cancel_url']) && !empty($info['cancel_url'])) {
            $this->data['cancel_url'] = $info['cancel_url']; // Use provided cancel URL
        } else {
            $this->setCancelUrl();
            $this->data['cancel_url'] = $this->getCancelUrl(); // Use config default
        }
